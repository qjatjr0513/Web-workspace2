...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    /*
	     * 폼전송을 일반방식이 아니라 multipart/form-data로 전송하는경우
	     * request로 값을 뽑을수 없음.
	     * multipart라는 객체로 값을 ㄱ이관시켜서 다뤄야한다.
	     *  
	     */
	    ...
	    if(ServletFileUpload.isMultipartContent(request)) {
	        ...
	        // 1_1. 전송파일 용량제한(int maxSize -> byte단위의 값을 기술) => 10Mbyte로 제한
	        ...
	        /*
	         * 세선객체에서 제공하는 getRealPath 메소드를 알아내기.
	         * 다만 WebContents폴더로부터 board_upfilres폴더까지의 경로는 매개변수로 제시해줘야함.
	         ...
	         * 그리고 사용자가 올린 파일명은 그대로 해당폴더에 업로드 하지 않은게 일반적임.
	         * - 같은파일명이 있을경우 에러가 날수있고, 한글, 특수문자, 띄어쓰기가 포함된 파일명일 경우 서버에따라 문제발생가능
	         * 
	         ...
	         * - 내입맛대로 나만의 파일명 생성규칙을 만들어서 바꿔주는 객체를 만들기.
	        ...
	        if(result > 0) { // 성공 => list.bo?currentPage=1
	            ...
	            response.sendRedirect(request.getContextPath()+"/list.bo"); // list.bo?currentPage=1"와 같음.
